...
- **Junction give-way implemented (heuristic)**: before any turn at a junction the van pauses 1.5 s and waits while a moving vehicle is within 25 m (own-lane lead and parked cars excluded); after 12 s of blocked waiting it creeps at 2 m/s rather than deadlocking. Limitations: radius-based conflict check, no lane-level right-of-way (a vehicle driving AWAY on the crossing road still counts as a conflict → occasional over-cautious waits), creep-on-timeout is a pragmatic policy that needs review before real roads (`va_intersection_crossing` scenarios upgrade from not_implemented to partial).
